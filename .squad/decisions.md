...
## Dashboard Manual-Trigger Concurrency and Run-State Contract

**Date:** 2026-09-22
**Status:** APPROVED

- Agents HQ per-agent triggers remain separate from Settings scheduler-task
  **Run Now** and do not mutate the `monitor_agents` task timestamp or its
  due-only `force_alpha=False` behavior.
- Dashboard executions are process-local records keyed by a server-generated
  run UUID. Trigger clients must poll the returned `run_id`; POST acceptance is
  not execution success.
- Manual-trigger slots are keyed by `(agent_type, symbol-or-wildcard)` and
  carry the run UUID as an immutable owner token. A worker releases a slot only
  when that token still owns it, preventing stale cleanup from deleting a
  replacement run.
- The registry, locks, and run records are eagerly initialized as one
  application process-state object. Test fallback initialization must publish
  that whole object atomically under one initialization lock.
- Thread construction and `start()` are transactional setup. A startup failure
  marks the exact run failed, releases its owned slot, and returns retryable
  non-success; after successful startup, worker cleanup owns the slot.
- Completions update only their own run under a lock. Per-agent aggregation is
  deterministic by newest start sequence, while `last_run` remains the newest
  successful completion through later running or failed attempts. The newest
  100 completed runs are retained and active runs are never pruned.

Rusty's initial trigger observability fix and Livingston's run-ID revision were
rejected for shared-state and slot races. Danny's owner-token revision plus
Rusty's transactional startup cleanup closed those races. Basher's final
dashboard gate approved the integrated behavior.

## Scrip Dividend Share-Leg FMV and FIFO Cost Basis

**Date:** 2026-09-22
**Status:** APPROVED

For `SCRIP_DIVIDEND` and the share leg of `DIVIDEND_WITH_SCRIP`,
`SHARE_ACQUISITION.gross.eur_amount` is the acquisition cost basis of the
received shares:

- positive finite EUR FMV → `COMPLETE` and included in FIFO lot cost;
- explicit numeric zero → `ZERO_COST` and a real zero-cost FIFO lot;
- blank authoritative EUR FMV → `INCOMPLETE`;
- negative, non-finite, or malformed supplied amounts → validation error;
- non-EUR native amount without `gross.eur_amount` remains preserved in its
  native currency but stays `INCOMPLETE`.

Create and group correction use the same backend normalization and never trust
submitted `cost_basis_status`; frontend shaping mirrors the rules only for
feedback. Legacy zero-valued `INCOMPLETE` legs are not silently reclassified.
Share FMV is neither dividend income nor `CASH_TOP_UP`; dividend totals continue
to use only the linked `CASH_DIVIDEND` leg.

Linus's accounting fix was initially held for adversarial validation gaps.
Livingston independently validated create/correction normalization and edge
cases, after which Basher approved the scrip gate.

## Production Blob Backup Cron, Tag Authorization, and RBAC Gate Disposition

**Date:** 2026-09-20
**Status:** APPROVED WITH USER-DIRECTED RBAC NON-BLOCKER

- The Container Apps backup Job runs once daily at `15 23 * * *` UTC
  (00:15 Madrid standard time, 01:15 daylight-saving time). Application
  local-date, due-time, and idempotency checks remain retry/manual-run guards,
  not polling.
- The production `AuthorizationPermissionMismatch` occurred on immutable
  tagged upload: container-scoped `Storage Blob Data Contributor` lacked Blob
  index-tag `tags/write`. A second tag-write-only custom role assigned to the
  runtime UAMI at the backup container restored production without changing
  image `sha-916faed`.
- Provisioning uses a deterministic role ID and validates existing definitions
  fail-closed. Diagnostics remain value-free; immutable creation, download/SHA
  verification, leases, CAS pointers, and tag-driven retention remain intact.
- Basher approved the cron and runtime/diagnostic portions but rejected the
  combined RBAC revision over broader `AssignableScopes` and inherited
  assignment validation. On 2026-09-20, the user explicitly directed that no
