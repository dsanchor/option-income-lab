...
- Gating applies to the **scheduled** run only: the watchlist queries (`get_covered_call_symbols` / `get_cash_secured_put_symbols` / `get_buy_tracker_symbols`) exclude symbols whose pause is still active (`watchlist_pause.until >= today`), so the cron job skips them. **Manual runs are always allowed** — the per-symbol path (the ▶ "Run analysis" button) runs even while paused, so you can generate activity on demand; it only respects the on/off watchlist flag, not the pause. Position monitors (Open Call / Open Put) are **not** affected.
- While paused, the symbol's rows on the main dashboard show `—` for all values (price, gap, recent, strike/expiry/premium or entry-zone/triggers) to avoid presenting stale data as current; any activity generated by a manual run is still visible on the symbol detail page.
