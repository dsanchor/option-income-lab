...
```json
{
  ...
  "rule_checks": {
    "ocm_a_assignment_risk": {"status": "pass|fail|warning|unknown", "detail": "brief"},
    "ocm_a_fundamental_check": {"status": "pass|fail|warning|unknown", "detail": "brief"}
  }
}
```
...
- `rule_checks` (REQUIRED): structured verdicts for qualitative rules. Include `ocm_a_assignment_risk` (is assignment rational/acceptable given ex-div, moneyness, and extrinsic value?) and `ocm_a_fundamental_check` (still comfortable owning the stock if assigned?). Each value is an object with `status` (one of `pass`, `fail`, `warning`, `unknown`) and a brief `detail` string. This block is consumed by the deterministic rule evaluator downstream — it does not change your narrative `reason`.
...
- `rule_checks` (REQUIRED): structured verdicts for qualitative rules, carried through to Phase 2's final output so the assessment-phase judgment persists. Include `ocm_a_assignment_risk` (is assignment rational/acceptable given ex-div, moneyness, and extrinsic value?) and `ocm_a_fundamental_check` (still comfortable owning the stock if assigned?). Each value is an object with `status` (one of `pass`, `fail`, `warning`, `unknown`) and a brief `detail` string. This block is consumed by the deterministic rule evaluator downstream — it does not change your narrative `reason`.
"""
