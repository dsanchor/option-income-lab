...
# ---------------------------------------------------------------------------
# 3.7 — Buy Tracker (A1.5: deterministic recalculation from enrichment_data)
# ---------------------------------------------------------------------------
...
def _enrichment_days_to_earnings(enrichment_data: Optional[Dict]) -> Optional[int]:
    """Recompute days-to-earnings from enrichment data, if available.

    Accepts either a pre-computed flat `next_earnings_date_days_away` (int) key,
    or the raw fetcher `overview` payload (JSON string/dict) from which the
    earnings date is derived and diffed against "now".
    """
    if not enrichment_data:
        return None
    flat = enrichment_data.get("next_earnings_date_days_away")
    if flat is not None:
        try:
            return int(flat)
        except (TypeError, ValueError):
            return None
    overview = _parse_json_payload(enrichment_data.get("overview"))
    fundamentals = overview.get("fundamentals") or {}
    earnings_field = fundamentals.get("earnings_release_next_date_fq") or {}
    formatted = earnings_field.get("formatted")
    if not formatted or formatted in ("N/A", "unknown"):
        return None
    earn_dt = _parse_date(formatted)
    if earn_dt is None:
        return None
    today = datetime.now(timezone.utc).replace(tzinfo=None)
    return (earn_dt - today).days


def _enrichment_rsi(enrichment_data: Optional[Dict]) -> Optional[float]:
    """Recompute RSI(14) from enrichment data, if available.

    Accepts either a pre-computed flat `rsi_14` key, or the raw fetcher
    `technicals` payload (JSON string/dict) from which RSI is extracted.
    """
    if not enrichment_data:
        return None
    flat = enrichment_data.get("rsi_14")
    if flat is not None:
        return _to_float(flat)
    technicals = _parse_json_payload(enrichment_data.get("technicals"))
    indicators = ((technicals.get("oscillators") or {}).get("indicators") or {})
    rsi_field = indicators.get("RSI") or {}
    return _to_float(rsi_field.get("value"))


def _bt_scoring_rule(rule_id: str, label: str, dimension_key: str, activity_data: Dict) -> Dict[str, Any]:
    breakdown = activity_data.get("score_breakdown") or {}
    value = breakdown.get(dimension_key)
    expected = f"Score 1 if {label.lower()} supports accumulation"
    if value is None:
        return _rule(
            rule_id, label, "scoring", STATUS_UNKNOWN, expected,
            "Score breakdown unavailable", "deterministic", blocking=False, data_refs={},
        )
...
def build_buy_tracker_rules(activity_data: Dict, enrichment_data: Optional[Dict]) -> List[Dict[str, Any]]:
    risk_flags = _risk_flags(activity_data)
    waiting_for_present = bool(activity_data.get("waiting_for"))
    activity_upper = _upper(activity_data.get("activity"))
    is_wait = activity_upper == "WAIT"
    ...
    # --- bt_wait_earnings: deterministic recalculation (A1.5) ---
    days_to_earnings = _enrichment_days_to_earnings(enrichment_data)
    expected_earnings = "No earnings within 2 days"
    if days_to_earnings is not None:
        triggered = days_to_earnings <= 2
        status = STATUS_BLOCKED if triggered else STATUS_NOT_APPLICABLE
        observed = (
            f"Earnings in {days_to_earnings} day(s) \u2014 recalculated from raw data"
            if triggered else f"Earnings in {days_to_earnings} day(s) \u2014 not triggered"
        )
        rules.append(_rule(
            "bt_wait_earnings", "WAIT: Earnings \u22642 Days", "trigger", status,
            expected_earnings, observed, "deterministic", blocking=triggered,
            data_refs={"days_to_earnings": days_to_earnings},
        ))
    else:
        # Fallback: trust LLM output signal (no raw data available)
        earnings_signal = is_wait and "calendar_risk_nearby" in risk_flags
        status = STATUS_BLOCKED if earnings_signal else STATUS_NOT_APPLICABLE if not is_wait else STATUS_UNKNOWN
        rules.append(_rule(
            "bt_wait_earnings", "WAIT: Earnings \u22642 Days", "trigger", status,
            expected_earnings,
            "Recalculation unavailable \u2014 using LLM signal" if is_wait else "Not triggered",
            "deterministic" if not is_wait else "hybrid",
            blocking=(status == STATUS_BLOCKED), data_refs={},
        ))

    # --- bt_wait_rsi_80: deterministic recalculation when available (A1.5) ---
    rsi_14 = _enrichment_rsi(enrichment_data)
    expected_rsi = "RSI \u2264 80"
    if rsi_14 is not None:
        triggered = rsi_14 > 80
        status = STATUS_BLOCKED if triggered else STATUS_NOT_APPLICABLE
        observed = f"RSI {rsi_14}" + (" \u2014 triggered" if triggered else " \u2014 not triggered")
        rules.append(_rule(
            "bt_wait_rsi_80", "WAIT: RSI >80", "trigger", status, expected_rsi, observed,
            "deterministic", blocking=triggered, data_refs={"rsi_14": rsi_14},
        ))
    else:
        rsi_signal = is_wait and "momentum_not_reset" in risk_flags
        status = STATUS_BLOCKED if rsi_signal else STATUS_NOT_APPLICABLE if not is_wait else STATUS_UNKNOWN
        rules.append(_rule(
            "bt_wait_rsi_80", "WAIT: RSI >80", "trigger", status, expected_rsi,
            "Recalculation unavailable \u2014 using LLM signal" if is_wait else "Not triggered",
            "hybrid" if is_wait else "deterministic",
            blocking=(status == STATUS_BLOCKED), data_refs={},
        ))

    # --- Remaining WAIT triggers: no raw recalculation source available (A1.5 table) ---
    extended_signal = is_wait and "entry_zone_not_reached" in risk_flags
    rules.append(_rule(
        "bt_wait_extended", "WAIT: Price Extended", "trigger",
        STATUS_BLOCKED if extended_signal else STATUS_NOT_APPLICABLE,
        "Not >10% above SMA50 AND >15% above SMA200",
        "Trigger signal present in risk_flags" if extended_signal else "Not triggered",
        "hybrid" if is_wait else "deterministic",
        blocking=extended_signal, data_refs={},
    ))

    div_cut_signal = "dividend_cut" in risk_flags or "dividend_cut_risk" in risk_flags
    rules.append(_rule(
        "bt_wait_div_cut", "WAIT: Dividend Cut", "trigger",
        STATUS_BLOCKED if div_cut_signal else STATUS_NOT_APPLICABLE,
        "No dividend cut/suspension",
        "dividend_cut flag present" if div_cut_signal else "Not triggered",
        "deterministic", blocking=div_cut_signal, data_refs={},
    ))

    triple_bear_signal = is_wait and waiting_for_present and "calendar_risk_nearby" not in risk_flags and (
        "momentum_not_reset" in risk_flags and "entry_zone_not_reached" in risk_flags
    )
    rules.append(_rule(
        "bt_wait_triple_bear", "WAIT: Triple Bearish", "trigger",
        STATUS_BLOCKED if triple_bear_signal else STATUS_NOT_APPLICABLE,
        "Not all three bearish indicators",
        "Combined bearish signals present" if triple_bear_signal else "Not triggered",
        "hybrid" if is_wait else "deterministic",
        blocking=triple_bear_signal, data_refs={},
    ))
