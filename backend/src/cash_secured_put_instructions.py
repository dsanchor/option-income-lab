...
- **category-params** (e.g., csp-aristocrat, csp-high-yield, etc.): MANDATORY — category-specific thresholds for delta, premium, IV, and market state evaluation. The symbol's category is provided in the prompt. Load the matching skill and apply its adjusted parameters. All base rules (earnings gate, DTE ≤ 45, fundamental checks) remain in force — the category skill ONLY adjusts thresholds.
...
### KEY PRINCIPLE
**The risk is NOT that earnings are nearby — the risk is that your position is OPEN during earnings.** Load **earnings-gate-sell** first and follow it before technical, IV, or premium analysis. For cash-secured puts, resolved post-earnings uncertainty can improve entries, but never override the DTE cap or the earnings gate.
